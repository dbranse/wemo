...
from keras.layers import Dense, Embedding, LSTM, LeakyReLU, Concatenate
...
def buildModel(semanticEmbeddingMatrix, sentimentEmbeddingMatrix):
    ...
    sentimentModel.add(sentimentEmbeddingLayer)
    sentimentModel.add(LSTM(LSTM_DIM, dropout=DROPOUT))

    semanticModel = Sequential()
    semanticModel.add(semanticEmbeddingLayer)
    semanticModel.add(LSTM(LSTM_DIM, dropout=DROPOUT))

    model = Sequential()
    model.add(Concatenate([sentimentModel, semanticModel]))
