...
# Stencil imports
import embeddings
from dataset import SentimentDataset, SentimentDatasetEval, SentimentDatasetLabeler
import hyperparams
...
def evaluate(hp, embedding_lookup):
    ...
    dataset = SentimentDatasetLabeler(args.data, mode)
    ...
    for (turn1, len1, turn3, len3) in tqdm(dataloader, ascii=True):
        len1 = len1.to(DEV)
        len3 = len3.to(DEV)
        model.eval()
        with torch.no_grad():
            ...
            label_sequence.append(predicted_label)
            labels[predicted_label] += 1

    print(labels)
    label2emotion = {0:"others", 1:"happy", 2: "sad", 3:"angry"}
    # File stuff
    with open('../labeledTrain.txt', 'a') as f:
        f.truncate(0)
        f.write("id\tturn1\tturn2\tturn3\tlabel\n")
        with open('../convo_collect/collected_convos.txt', 'r') as data:
            lines = data.readlines()
            for id, label in enumerate(label_sequence):
                input = lines[id]
                full_line = str(id) + "\t" + input.replace('\n','') + "\t" + label2emotion[label] + "\n"
                f.write(full_line)
