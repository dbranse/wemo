...
def getSentimentEmbeddingMatrix(wordIndex):
    """Populate an embedding matrix using a word-index. If the word "happy" has an index 19,
       the 19th row in the embedding matrix should contain the embedding vector for the word "happy".
    Input:
        wordIndex : A dictionary of (word : index) pairs, extracted using a tokeniser
    ...
    embeddingsIndex = {}
    # Load the embedding vectors from ther GloVe file
    with io.open(os.path.join(gloveDir, 'formattedSentiment.txt'), encoding="utf8") as f:
        for line in f:
            values = line.split()
            word = values[0]
            embeddingVector = np.asarray(values[1:], dtype='float32')
...
def buildModel(semanticEmbeddingMatrix, sentimentEmbeddingMatrix):
    ...
    sentimentEmbeddingLayer = Embedding(sentimentEmbeddingMatrix.shape[0], 
                                        EMBEDDING_DIM,
                                        weights=[sentimentEmbeddingMatrix],
                                        input_length=MAX_SEQUENCE_LENGTH,
                                        trainable=False)(inp)
    sentimentOut = LSTM(LSTM_DIM, dropout=DROPOUT)(sentimentEmbeddingLayer)
    ...
    concat = concatenate([sentimentOut, semanticOut])
...
def main():
    ...
    sentimentMatrix = getSentimentEmbeddingMatrix(wordIndex)
    ...
    for k in range(NUM_FOLDS):
        ...
        model = buildModel(embeddingMatrix, sentimentMatrix)
    ...
    model = buildModel(embeddingMatrix, embeddingMatrix)
